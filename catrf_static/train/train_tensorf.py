...
# Make catrf_static root findable regardless of where this script is called from.
_CATRF_STATIC_ROOT = Path(__file__).resolve().parents[1]
if str(_CATRF_STATIC_ROOT) not in sys.path:
...
def reconstruction(args):
    ...
    for it in pbar:
        ...
        if args.compression:
            ...
            den_rate = sum((torch.log(l).sum() / (-math.log(2))) for pack in den_like for l in pack["likelihoods"].values())
            app_rate = sum((torch.log(l).sum() / (-math.log(2))) for pack in app_like for l in pack["likelihoods"].values())
