...
class LinearPack(nn.Module):
    """
    12->3 encoder (1x1), optional DW 3x3, bounded to [0,1].
    3->12 decoder is tied ~ pseudo-inverse of encoder, plus optional DW 3x3.
    """
    def __init__(self, in_ch=12, mid_dw=False, eps=1e-3):
        super().__init__()
        self.enc = nn.Conv2d(in_ch, 3, 1, bias=True)
        self.dec = nn.Conv2d(3, in_ch, 1, bias=True)

        # optional shallow spatial smoothing/sharpening (codec-friendly)
        self.pre_dw  = nn.Conv2d(3, 3, 3, padding=1, groups=3, bias=False) if mid_dw else None
        self.post_dw = nn.Conv2d(3, 3, 3, padding=1, groups=3, bias=False) if mid_dw else None

        # bounded projector on RGB side (keep in [0,1])
        self.bound = BoundedProjector(3, eps=eps)

        self._init_tied()

    @torch.no_grad()
    def _init_tied(self):
        # Start near identity: pick 3 channels out of 12 (or small PCA), and set dec ≈ pinv(enc)
        W = torch.zeros(3, 12, 1, 1)
        W[0, 0] = 1.0; W[1, 1] = 1.0; W[2, 2] = 1.0  # trivial seed (can swap to PCA later)
        self.enc.weight.copy_(W)
        self.enc.bias.zero_()
        # pseudo-inverse for conv1x1: (W^T * (W W^T + λI)^{-1})
        lam = 1e-3
        W2d = W.view(3, 12)
        pinv = W2d.t() @ torch.inverse(W2d @ W2d.t() + lam*torch.eye(3))
        self.dec.weight.copy_(pinv.view(12, 3, 1, 1))
        self.dec.bias.zero_()
        if self.pre_dw is not None:
            nn.init.dirac_(self.pre_dw.weight)
            nn.init.dirac_(self.post_dw.weight)

    def forward_pre(self, x01):  # [B,12,H,W] -> [B,3,H,W] in [0,1]
        y = self.enc(x01)
        if self.pre_dw is not None:
            y = self.pre_dw(y)
        y = self.bound(y)
    ...
    def forward_post(self, y01):  # [B,3,H,W] -> [B,12,H,W] (≈[0,1])
        z = y01
        if self.post_dw is not None:
            z = self.post_dw(z)
        x_rec = self.dec(z)
        return x_rec.clamp_(0, 1)

    # optional soft tying loss (keeps dec close to pinv(enc))
    def tying_reg(self):
        with torch.no_grad():
            W = self.enc.weight.view(3, 12)
            lam = 1e-3
            pinv = W.t() @ torch.inverse(W @ W.t() + lam*torch.eye(3, device=W.device))
            P = pinv.view_as(self.dec.weight)
        return F.mse_loss(self.dec.weight, P)
